...
export async function buscarIngestoes(db: SQLite.SQLiteDatabase): Promise<Ingestao[]> {
    return await db.getAllAsync<Ingestao>(`
        SELECT 
            id,
            qnt_ml,
...
export async function addIngestao(db: SQLite.SQLiteDatabase, quantidadeIngerida: number): Promise<void> {
    await db.runAsync(
...
export async function atualizarIngestao(
    ...
    await db.runAsync(
        `UPDATE ingestao 
         SET qnt_ml = ?, horario = datetime(?, '+3 hours')
         WHERE id = ?;`,
        [novaQuantidade, novoHorario, id]
    );
}

export async function deletarIngestao(db: SQLite.SQLiteDatabase, id: number): Promise<void> {
    await db.runAsync(
...
export async function buscarProfile(db: SQLite.SQLiteDatabase): Promise<Profile | null> {
    ...
    await db.runAsync(
        'UPDATE profile SET consumo_parcial = ? WHERE id = 1;',
        [totalHoje]
    );
